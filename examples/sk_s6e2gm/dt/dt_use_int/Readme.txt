...
The interval time for channel 0 is 0.5 sec at first. Then interval time changes
to 1 sec.
...
2) Watch the Terminal I/O window (Debug viewer)
   If a character isn't output to the Terminal I/O window (Debug viewer), the DT is operating
   normally.
3) PB2 (green LED) outputs HIGH after 0.5 sec, then this port outputs HIGH and LOW for every
   second (according to DT channel 0).
   Watch the variable "u32CountDt1" for DT channel 1 interrupt count.
