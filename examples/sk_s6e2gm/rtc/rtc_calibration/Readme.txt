...
In this example first initialized the calibration function of the Real Time Clock. 
The original 1 Hz signal divided by subclock is output from SUBOUT_0, and the
CO signal which was calibrated with the frequency correction module is
output from RTCCO_1.
Then the RTC calibration value is changeed in 20 steps.
A frequency tester with high accuracy can be used to monitor the output
change from RTCCO_1 when implementing the calibration function.
...
* Run the code
* Watch the waveform from RTCCO_1 and SUBOUT_0
* Run the code again
* Watch the frequency change from RTCCO_1 with a frequency tester.
