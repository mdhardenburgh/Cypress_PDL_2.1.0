...
#if !defined(SetPinFunc_TIOB0_2_IN)
#error TIOB0_2 BT2 pins are not available in this MCU product, \
change to other re-location pins or BT channels! Then delete "me" ! \
Note that the pin name in some product is TIOB00_2, thus SetPinFunc_TIOB00_2_IN \
should be called for such product!
#endif

/* The PWM output is used as measured wave for PWC timer, connect P42(TIOA2_0) 
* with P11(TIOB0_2) before running the program. */
/* PWM  */
...
#define  PwcInitInput()         SetPinFunc_TIOB0_2_IN();
