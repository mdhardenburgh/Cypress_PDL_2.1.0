...
#ifdef DEBUG_PRINT
                printf("CR_TRIMMING_PROC_TMAXFINE_MEAS_FINISH\n");
#endif
                u32MeasData = 0;
            ...
            case CR_TRIMMING_PROC_TUNE_FINE_VAL: 
                ...
                while(m_u32CntIrqMeasure < 10)
                {
                    if(m_u32CntIrqOverflow)
                    {
                        ...
                        Bt_Pwc_DisableCount(&BT0);
#ifdef DEBUG_PRINT
                    printf("CR_TRIMMING_RET_ERR!!! m_u32CntIrqMeasure = %"PRIu32"\n", m_u32CntIrqOverflow);
#endif                        
                        return CR_TRIMMING_RET_ERR;
                    }
#ifdef DEBUG_PRINT
                    printf("m_u32CntIrqMeasure = %"PRIu32"\n", m_u32CntIrqMeasure);
#endif
                }
#ifdef DEBUG_PRINT
                printf("m_u32CntIrqMeasure = %"PRIu32"\n", m_u32CntIrqMeasure);
                ...
                if(u8TrimState == CR_TRIMMING_PROC_TUNE_COARSE_VAL)
                {
                    ...
                    if(f32FinalCycle < 1/f32ExpCrFreq) /* Freq > 4MHz, continue to tune */
                    {
                        ...
                        {
#ifdef DEBUG_PRINT
                    printf("CR_TRIMMING_RET_ERR!!! u8CoarseData = %d\n", u8CoarseData);
#endif                      
                            return CR_TRIMMING_RET_ERR;
                        }
                ...
                else
                {
                    ...
                    if(f32FinalCycle < 1/f32ExpCrFreq) 
                    {
                        ...
                        {
#ifdef DEBUG_PRINT
                    printf("CR_TRIMMING_RET_ERR!!! u8FineData = %d\n", u8FineData);
#endif 
                            return CR_TRIMMING_RET_ERR;
                        }
