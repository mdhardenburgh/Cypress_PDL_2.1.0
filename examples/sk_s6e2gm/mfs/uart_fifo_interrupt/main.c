...
* \version          1.10
*
* \brief            Demo the communication between UART7 and UART6
*
********************************************************************************
* \copyright
...
stc_uart_irq_cb_t stcUart7IrqCb, stcUart6IrqCb;
...
/******************************************************************************/
volatile stc_mfsn_uart_t* UartCh7 = &UART7;
volatile stc_mfsn_uart_t* UartCh6 = &UART6;
static uint8_t au8UartTxBuf[200] = "Data sent from UART7 to UART6";
...
/*******************************************************************************
* Function Name: Uart7FifoTxIntCallback
********************************************************************************
* UART7 FIFO TX interrupt callback function
*******************************************************************************/
void Uart7FifoTxIntCallback(void)
{
    uint8_t u8i = 0;
    if(stcTxFifoInfo.u32TxCnt > SAMPLE_UART_FIFO_MAX_CAPACITY)
    {
        ...
        {
            Mfs_Uart_SendData(UartCh7, *stcTxFifoInfo.pTxBuf++);
            u8i++;
        }
    ...
    Mfs_Uart_DisableIrq(UartCh7, UartTxFifoIrq);
...
********************************************************************************
* UART7 RX interrupt callback function
*******************************************************************************/
...
void InitUart(void)
{
    ...
    stcUart7IrqCb.pfnTxFifoIrqCb = Uart7FifoTxIntCallback;
    ...
    stcUartConfig.pstcIrqCb = &stcUart7IrqCb;
    stcUartConfig.bTouchNvic = TRUE;
    Mfs_Uart_Init(UartCh7, &stcUartConfig);
...
{   
    ...
    /* Enable TX function of UART7 */
    Mfs_Uart_EnableFunc(UartCh7, UartTx);
    
    /* Configure interrupt */    
    Mfs_Uart_EnableIrq(UartCh6, UartRxIrq);
    Mfs_Uart_EnableIrq(UartCh7, UartTxFifoIrq);
